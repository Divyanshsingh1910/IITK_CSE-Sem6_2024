...
char *tokens[] = {"NULL","KEYWORD","NAME","INT_LITERAL","REAL_LITERAL","LOGICAL_LITERAL","CHAR_LITERAL","OPERATOR","LABEL","DELIMITER","SPECIAL_CHAR","COMMENT","STR_ERROR","ERROR"};
	...
	*/	
	...
	if(!lex_stream){
		...
		while(ptr){
			...
			// 	printf("eND abhi aaya hai\n");
			// }
			int capital_diff = strcmp(str1,str2);     //comparison for all upper_cased version
			free(str1); free(str2);

			int diff = strcmp(ptr->parent_lexeme,text);  
			// if(strcmp(text,"eND")==0){
			// 	printf("compared with {%s} and diff is %d and prev count is %d\n",ptr->parent_lexeme,diff,ptr->count);
			// }
			//printf("Comparing %s now\n",text);
			...
			if(capital_diff == 0){
				...
				// if(strcmp(text,"eND")==0){
				// 	printf("compared with {%s} and diff is %d and prev count is %d and new count is set to %d\n",ptr->parent_lexeme,diff,ptr->count,new_count);
				// }
				update_count(new_count,text);

				// if(strcmp(text,"eND")==0){
				// 	printf("New count of %s is %d\n",ptr->parent_lexeme,ptr->count);
				// }
...
int contains_ext(char* filestr){
	...
	if(where_dot == NULL || strcmp(where_dot, ".f08")){
		return 0;
	}
